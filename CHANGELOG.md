...
#### 个人资料库
- 新增独立一级功能板块“资料库”，用于集中管理保研材料、证明文件、简历、自荐信、面试资料等本地文件和文件夹
- 支持添加单个或多个文件，支持添加文件夹路径
- 支持 Excel、PPT、Word、PDF、Markdown、压缩包、图片、文件夹等多种资料形式
- 自动记录资料名称、路径、文件类型、大小、修改时间，并支持分类、标签和备注
- 支持按关键词、分类、文件/文件夹类型筛选资料
- 支持一键调用系统默认程序打开资料或文件夹

#### 院校看板
- 支持院校在冲/稳/保三个分组之间拖拽转换
- 跨分组移动时同步更新院校设置中的申请层次，例如从“冲”拖到“稳”后自动改为“稳”
- 保留同组内拖拽排序能力，并继续持久化排序结果

### 数据与备份
- 新增 `PersonalResource` 数据表保存资料库记录
- 启动时自动为旧数据库无损补齐资料库表
- 备份导出/导入包含资料库记录（保存路径和元数据，不复制原始文件内容）
...
#### 院校看板
- 院校卡片支持在同一冲/稳/保分组内拖拽排序
- 排序结果持久保存，刷新或重启软件后仍保留自定义顺序
- 拖拽过程中增加卡片和分组高亮反馈

### 数据与更新
- 新增 `sortOrder` 字段用于保存院校显示顺序
- 启动时自动为旧数据库无损补齐 `sortOrder` 字段
- 更新 GitHub Release 上传规则，确保 Windows 自动更新所需的 `latest.yml` / blockmap 一起发布
