...
2. 在最新版本（v3.0.0）的 Assets 中，找到 **`PG-Tracker-3.0.0-win-setup.exe`**
3. 点击下载
...
5. **汇总资料** — 在"资料库"页面添加文件或文件夹，按分类、标签和备注建立自己的保研资料数据库
6. **记录面经** — 面试结束后，在导师卡片上点击"记录面经"，用 Markdown 记录面试内容
7. **发送邮件** — 在"邮件模板"页面选择模板，填入变量，一键复制后粘贴到邮箱
8. **备份数据** — 在"设置"页面随时导出 JSON 备份，换电脑时可导入恢复
...
每张院校卡片展示：院校名称、院系、学位类型（学硕/直博）、招生名额、特殊政策标签、截止日期倒计时、导师数量。支持按等级标签筛选（全部/冲/稳/保），支持同组内拖拽排序，也支持在冲/稳/保之间拖拽转换；跨组移动时会同步更新院校设置中的申请层次。
...
独立的资料库页面用于集中管理保研全过程中的本地材料，适合放置：

- 申请材料汇总、成绩排名证明、科研竞赛获奖证书
- 推荐信、学生证和身份证复印件、英语水平证明材料
- 照片、自荐信和简历、自我介绍、面试资料
- Excel、PPT、Word、PDF、Markdown、压缩包、图片和文件夹

资料库会保存资料路径和元数据，包括名称、类型、分类、标签、备注、大小和修改时间。支持关键词搜索、分类筛选、文件/文件夹筛选，并可一键调用系统默认程序打开对应文件或文件夹。备份导出会保存资料库记录，但不会复制原始文件内容。
...
- **主题切换**：浅色 / 深色 / 跟随系统
- **数据导出**：将所有数据导出为 JSON 文件备份（包含院校、导师、任务、邮件模板和资料库记录）
- **数据导入**：从 JSON 备份文件恢复数据（完整恢复院校、导师、任务、邮件模板、资料库等所有数据）
